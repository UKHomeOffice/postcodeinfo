...
class PostcodeGssCodeImporterTestCase(TestCase):
  ...
  def __import_data_from(self, file):
    self.__importer().import_postcode_gss_codes(self.__sample_data_file(file))

  def test_that_postcode_gss_code_objects_get_the_right_attributes(self):
    self.__import_data_from('NSPL_sample.csv')
    postcode_gss_code = PostcodeGssCode.objects.filter(postcode_index='ab10ad').first()
    self.assertEqual(postcode_gss_code.local_authority_gss_code, 'S12000033')
