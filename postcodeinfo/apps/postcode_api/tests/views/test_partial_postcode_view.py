...
class PartialPostcodeViewTestCase(TestCase):
  def setUp(self):
    self.user = User()
    self.user.save()
  ...
  def test_that_getting_a_valid_partial_postcode_with_a_valid_token_produces_parseable_json(self):
    response = self.client.get('/postcodes/partial/N2', {}, True, False, HTTP_AUTHORIZATION=self.valid_token )
    parsed = json.loads(response.content) 
    self.assertIsInstance(parsed, dict)

  def test_that_getting_a_valid_partial_postcode_with_a_valid_token_responds_with_HTTP_200(self):
    response = self.client.get('/postcodes/partial/N2', {}, True, False, HTTP_AUTHORIZATION=self.valid_token )
    self.assertEqual(response.status_code, 200)

  def test_that_getting_an_invalid_partial_postcode_with_a_valid_token_responds_with_HTTP_404(self):
    response = self.client.get('/postcodes/partial/N2MUPP37', {}, True, False, HTTP_AUTHORIZATION=self.valid_token )
    self.assertEqual(response.status_code, 404)

  def test_that_getting_a_valid_partial_postcode_with_an_invalid_token_responds_with_HTTP_401(self):
    response = self.client.get('/postcodes/partial/N2', {}, True, False, HTTP_AUTHORIZATION='Token 1234' )
