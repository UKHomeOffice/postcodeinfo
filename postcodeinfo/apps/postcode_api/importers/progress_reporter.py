...
class ProgressReporter(object):
    ...
    def start(self, filename):
        ...
        print "starting import of %i lines at %s" % (self.total_lines, self.human_time())

    def row_processed(self, row_identifier=''):
        self.lines_processed += 1
        cumulative_time = self.time_taken()
        print "%s (%s taken), processed: %i, remaining: %i, time_per_row: %f, est. time remaining %s, %s" %  \
            (self.human_time(), 
    ...
    def finish(self):
        ...
        print '%i lines processed in %s' % (self.lines_processed, self.hours_minutes_seconds(self.time_taken()))
